...
async function loadRows() {
  ...
  tbody.innerHTML = data.map((row) => `
    <tr data-row="${row.id}">
      <td>${escapeHtml((row.quote || "").slice(0, 80))}${row.quote && row.quote.length > 80 ? "&hellip;" : ""}</td>
      <td>${escapeHtml(row.author_name)}${row.company ? ", " + escapeHtml(row.company) : ""}</td>
      <td><span class="badge ${row.is_published ? "yes" : "no"}">${row.is_published ? "Published" : "Hidden"}</span></td>
      <td class="actions">
  ...
  tbody.querySelectorAll("[data-edit-btn]").forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      form.author_name.value = row.author_name || "";
      form.author_role.value = row.author_role || "";
